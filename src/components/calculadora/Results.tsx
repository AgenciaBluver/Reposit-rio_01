      ...
      <div className="p-5 sm:p-6">
        <div className="flex items-baseline justify-between gap-3">
          <span className="bv-eyebrow text-fg-muted">
            {loss ? "Prejuízo" : "Lucro"} por {result.units > 1 ? "anúncio" : "produto"}
          </span>
        ...
        <p className="mt-2 text-[0.75rem] leading-snug text-fg-muted">
          O que sobra do preço depois do custo do produto e de tudo que o Mercado Livre e o
          imposto levam.
        </p>
        <hr className="bv-rule-signal mt-4 opacity-60" />

        <dl className="mt-5 grid grid-cols-2 gap-x-4 gap-y-4">
          <Metric label="Margem de lucro" value={pct(result.marginPct)} />
          {result.units > 1 && (
            <Metric label="Lucro por unidade" value={brl(result.profitPerUnit)} />
          )}
          <Metric label="Markup" value={`${result.markup.toFixed(2)}×`} />
          <Metric label="Lucro por hora de impressora" value={brl(result.profitPerPrintHour)} />
          <Metric
            label={result.units > 1 ? "Custo do kit" : "Custo do produto"}
            value={brl(result.production.total)}
          />
          <Metric label="Taxas do Mercado Livre" value={brl(result.marketplaceTotal)} />
        </dl>
      </div>

      {/* ── Composição do preço ── */}
      <div className="border-t border-fg/12 p-5 sm:p-6">
        <span className="bv-eyebrow text-fg-muted">Para onde vai cada real do preço</span>
      ...
      <div className="border-t border-fg/12 p-5 sm:p-6">
        <div className="flex items-baseline justify-between gap-3">
          <label htmlFor="alvo" className="bv-eyebrow text-fg-muted">
            Preço para a margem de lucro desejada
          </label>
...
export function Breakdown({
  ...
  const rows: { label: string; value: number; note?: string }[] = [
    ...
    { label: "Embalagem", value: p.packaging, note: "Uma por anúncio, não por peça" },
  ...
  const saleRows: { label: string; value: number; note: string }[] = [
    {
      ...
      value: p.total,
      note: kit
        ? `As ${p.units} peças do anúncio: material, eletricidade, máquina, perdas, insumos e embalagem`
        : "Material, eletricidade, máquina, perdas, insumos e embalagem",
    },
    {
    ...
    },
    {
      ...
      note: "Armazenagem no Full, coleta e deslocamento — uma vez por anúncio",
    },
    {
      label: "Imposto",
      value: result.tax,
  ...
  return (
    <div className="grid gap-gutter lg:grid-cols-[minmax(0,1fr)_22rem]">
      ...
      <div className="border border-fg/12 bg-fg/[0.03] p-5 sm:p-6">
        <h2 className="bv-display text-[1.0625rem] font-medium">Custo do produto, linha a linha</h2>
        {kit && (
        ...
        <table className="mt-4 w-full text-[0.875rem]">
          <caption className="sr-only">Composição do custo de um produto</caption>
          <tbody>
            {rows.map((r) => (
            ...
            <tr className="border-t border-fg/25">
              <th scope="row" className="py-3 text-left font-medium">
                Custo do produto{kit ? " (anúncio inteiro)" : ""}
              </th>
      ...
      <div className="border border-fg/12 bg-fg/[0.03] p-5 sm:p-6">
        ...
        <p className="mt-2 max-w-[62ch] text-[0.8125rem] leading-relaxed text-fg-muted">
          Tudo que sai do preço antes de sobrar lucro. As linhas zeradas continuam à vista de
          propósito: saber por que uma taxa não está sendo cobrada vale tanto quanto saber o
          valor das outras.
        </p>
        <table className="mt-4 w-full text-[0.875rem]">
          ...
          <tbody>
            ...
            <tr className="border-t border-fg/25">
              <th scope="row" className="py-3 text-left font-medium">
                {loss ? "Prejuízo" : "Lucro"} por {kit ? "anúncio" : "produto"}
              </th>
              <td
